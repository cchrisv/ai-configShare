...
Runtime artifacts are stored in `.ai-artifacts/<work_item_id>/` (research, grooming, solutioning, finalization, wiki-content.md; dev-updates when using phase-02c/03c, closeout when using phase-06, solution-design when using util-feature-solution-design, sequencing when using util-sequence-tickets). Feature research artifacts are stored in `.ai-artifacts/sf-research/<feature-name>/`.
...
Supporting/shared: `util-base.prompt.md`, `util-research-base.prompt.md`, `util-wiki-base.prompt.md`

### Phase Prompts

- `phase-01-initialize.prompt.md` - Create artifact folder, initialize workflow, scaffold + publish wiki skeleton
- `phase-02a-grooming-research.prompt.md` - Business research (org dictionary, ADO, wiki, business context)
- `phase-02b-grooming.prompt.md` - Requirements refinement (what & why) + fill wiki What sections
- `phase-02c-grooming-update.prompt.md` - Iterative requirements/scope updates (standalone) + re-fill wiki What
- `phase-03a-solutioning-research.prompt.md` - Technical research (Salesforce metadata, web best practices) + fill wiki Why discovery
- `phase-03b-solutioning.prompt.md` - Solution design (how) + fill wiki Why decisions, How solution
- `phase-03c-solutioning-update.prompt.md` - Iterative solution design updates (standalone) + re-fill wiki solutioning sections
- `phase-04-test-cases.prompt.md` - Test case generation (Salesforce Test Cases Playbook) + fill wiki How quality
- `phase-05-finalization.prompt.md` - Final validation, ADO update + wiki footer & validation
...
**Wiki:** Scaffolded in Phase 01; sections filled incrementally by each subsequent phase using `<!-- SECTION:id -->` markers. No discrete wiki phase — content is generated alongside the work that produces it.
