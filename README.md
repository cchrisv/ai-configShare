...
├── cli/                  # CLI entry points (workflow-tools, ado-tools, sf-tools, wiki-tools, pr-tools, report-tools, team-tools, template-tools)
├── src/                  # Source code
└── dist/                 # Compiled output (generated by npm run build)
```

Runtime artifacts are stored in `.ai-artifacts/<work_item_id>/ticket-context.json` — a single unified context file that progressively grows through phases. Feature research artifacts are stored in `.ai-artifacts/sf-research/<feature-name>/`.
...
- `util-pr-analysis.prompt.md` - Analyze PR diffs, evaluate code quality, and compare changes against user story requirements
...
Artifacts are stored in `.ai-artifacts/sf-research/<feature-name>/` with a shared `research-context.json` across all phases.
...
npx --prefix scripts/workflow pr-tools get --url <fullPrUrl> --json
...
npx --prefix scripts/workflow template-tools scaffold-phase --phase <phase> --type "User Story" -w <id> --json
```
...
#   --sf-org <alias>        Include Salesforce login/metadata activity
...
```bash
# Get PR metadata
npx --prefix scripts/workflow pr-tools get <prId> --repo <repoId> [--json]
npx --prefix scripts/workflow pr-tools get --url <fullPrUrl> [--json]

# Get PR diff
npx --prefix scripts/workflow pr-tools diff <prId> --repo <repoId> [--file <path>] [--max-files <n>] [--mode <mode>] [--json]
npx --prefix scripts/workflow pr-tools diff --url <fullPrUrl> [--json]

# Get PR review threads
npx --prefix scripts/workflow pr-tools threads <prId> --repo <repoId> [--json]
npx --prefix scripts/workflow pr-tools threads --url <fullPrUrl> [--json]

# Get linked work items
npx --prefix scripts/workflow pr-tools work-items <prId> --repo <repoId> [--json]

# List PRs linked to a work item
npx --prefix scripts/workflow pr-tools list --work-item <id> [--json]
```

**Diff modes:** `context` (default, source + diff), `full` (source + target + diff), `diff-only` (unified diff only).

### template-tools

Template scaffolding, rendering, and validation.

```bash
# List available templates for a phase and work item type
npx --prefix scripts/workflow template-tools list --phase <phase> --type "User Story" [--json]

# Scaffold fill spec for a phase (generates slot shapes the AI fills)
npx --prefix scripts/workflow template-tools scaffold-phase --phase <phase> --type "User Story" -w <id> --context <file> [--json]

# Render filled slots to HTML
npx --prefix scripts/workflow template-tools render-phase --phase <phase> -w <id> --context <file> [--json]

# Validate rendered HTML against template
npx --prefix scripts/workflow template-tools validate --template <key> --rendered <file> [--json]

# Get template info (slots, structure)
npx --prefix scripts/workflow template-tools info --template <key> [--json]
```
