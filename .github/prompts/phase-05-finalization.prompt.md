...
A2 [IO]: Load {{context_file}} → verify:
  ...
  - `.metadata.phases_completed` includes `"test_cases"`
...
D9 [LOGIC]: If any validation fails:
  ...
  - Tell user: **"⚠️ Test suite completeness issues detected. Consider re-running /phase-04-test-cases before finalizing."**
...
## Step 8 [IO/GEN/CLI] – Wiki: Footer + Validation
Ref: `#file:.github/prompts/util-wiki-base.prompt.md`

H1 [IO]: Read `{{root}}/wiki-content.md`
H2 [GEN]: Replace content between `<!-- SECTION:footer -->` and `<!-- /SECTION:footer -->` with final footer:
  - `📅 **Last Updated:** {{current_ISO_timestamp}}`
  - `Documentation generated by AI-assisted workflow.`
H3 [LOGIC]: Validate all `sections_generated` values are `true`. If any are `false`, log warning to `run_state.errors[]` — do not block completion.
H4 [IO]: Write updated `{{root}}/wiki-content.md`
H5 [CLI]: `{{cli.wiki_update}} --path "{{wiki_path}}" --content "{{root}}/wiki-content.md" --json`
H6 [IO]: Append `{"phase":"finalization","step":"wiki_footer","completedAt":"<ISO>"}` to `run_state.completed_steps[]`
